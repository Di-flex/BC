...
function KinkyDungeonPlayerIsVisibleToJailers() {
	for (let enemy of KinkyDungeonEntities) {
		if (!enemy.Enemy.allied && !enemy.rage && (enemy.Enemy.tags.has('jail') || enemy.Enemy.tags.has('jailer'))
			&& KinkyDungeonCheckLOS(enemy, KinkyDungeonPlayerEntity, KDistChebyshev(KinkyDungeonPlayerEntity.x - enemy.x, KinkyDungeonPlayerEntity.y - enemy.y), enemy.Enemy.visionRadius, false, false)) {
			return true;
		}
	}
	return false;
...
function KinkyDungeonAggroAction(action, data) {
	switch (action) {
		...
		case 'chest':
			if (KinkyDungeonPlayerIsVisibleToJailers())
				KDGameData.PrisonerState = "";
			break;
...
function KinkyDungeonPlaceJailKeys() {
	...
	for (let X = 1; X < KinkyDungeonGridWidth; X += 1)
		for (let Y = 1; Y < KinkyDungeonGridHeight; Y += 1)
			if (KinkyDungeonGroundTiles.includes(KinkyDungeonMapGet(X, Y)) && KDistChebyshev(X - KinkyDungeonPlayerEntity.x, Y - KinkyDungeonPlayerEntity.y) > 10
