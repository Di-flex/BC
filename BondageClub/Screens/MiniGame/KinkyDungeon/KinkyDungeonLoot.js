...
function KinkyDungeonLootEvent(Loot, Floor, Replacemsg, Lock) {
	...
	/**
	 * extracted internal method. May consider changing it to use map<string,()=>{...}>
	 */
	function KDHandleLootEventCases() {
		...
		if (Loot.weapon) {
			...
			KinkyDungeonSendFloater({
				x: 1100,
				y: 800 - KDRecentRepIndex * 40
			}, `+${amount} Spell Points!!!`, "#8888ff", 5, true);
			KDRecentRepIndex += 1;
			if (Replacemsg)
