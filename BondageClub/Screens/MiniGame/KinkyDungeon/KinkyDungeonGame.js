...
function KinkyDungeonPlaceChargers(chargerlist, chargerchance, litchargerchance, chargercount, Floor, width, height) {
	...
	while (list.length > 0) {
		...
		if (count <= chargercount) {
			...
			if (tile != '-') {
				KinkyDungeonTiles.set("" + charger.x + "," +charger.y, {Type: "Charger", NoRemove: tile == '=', Light: (tile == '=' ? KDChargerLight : undefined)});
			}
...
function KinkyDungeonLaunchAttack(Enemy, skip) {
	...
	if (KinkyDungeonHasStamina(Math.abs(attackCost), true)) {
		if (!KDGameData.ConfirmAttack && (!KinkyDungeonAggressive(Enemy) || KDAllied(Enemy))) {
			...
			KinkyDungeonAttackEnemy(Enemy, {damage: KinkyDungeonPlayerDamage.dmg, type: KinkyDungeonPlayerDamage.type, bind: KinkyDungeonPlayerDamage.bind, bindEff: KinkyDungeonPlayerDamage.bindEff, boundBonus: KinkyDungeonPlayerDamage.boundBonus, tease: KinkyDungeonPlayerDamage.tease});
...
function KinkyDungeonMove(moveDirection, delta, AllowInteract, SuppressSprint) {
	...
	let allowPass = Enemy && !Enemy.Enemy.immobile && ((!KinkyDungeonAggressive(Enemy) && !Enemy.playWithPlayer) || (KDHelpless(Enemy))) && (KinkyDungeonToggleAutoPass || KDEnemyHasFlag (Enemy, "passthrough") || (KinkyDungeonFlags.has("Passthrough")) || Enemy.Enemy.noblockplayer);
	if (Enemy && !allowPass) {
		if (AllowInteract) {
