...
function KinkyDungeonDamageEnemy(Enemy, Damage, Ranged, NoMsg, Spell, bullet, attacker, Delay, noAlreadyHit) {
	...
	if (!Damage && predata.type != "inert" && dmgDealt <= 0) {
		...
		if (KDRandom() < actionDialogueChanceIntense)
			KinkyDungeonSendDialogue(Enemy, TextGet("KinkyDungeonRemindJail" + (Enemy.Enemy.playLine ? Enemy.Enemy.playLine : "") + "MissedMe").replace("EnemyName", TextGet("Name" + Enemy.Enemy.name)), KDGetColor(Enemy), 4, 5);
	...
	for (let E = 0; E < KinkyDungeonBullets.length; E++) {
		...
		if ((Allied && b.bullet && b.bullet.spell && !b.bullet.spell.enemySpell) || (!Allied && !(b.bullet && b.bullet.spell && !b.bullet.spell.enemySpell))) {
			...
			while (d > 0.1) {
				...
				let checkCollision = (b.bullet.faction == "Player" && (b.x != KinkyDungeonPlayerEntity.x || b.y != KinkyDungeonPlayerEntity.y)) || justBorn || b.x != startx || b.y != starty || (!b.vx && !b.vy) || (KDistEuclidean(b.vx, b.vy) < 0.9); // Check collision for bullets only once they leave their square or if they are slower than one
...
function KinkyDungeonBulletHit(b, born, outOfTime, outOfRange, d) {
	...
	if (b.bullet.hit == "") {
		...
		if (b.bullet.effectTile) {
			KDCreateAoEEffectTiles(b.x, b.y, b.bullet.effectTile, b.bullet.effectTileDurationMod, (b.bullet.spell.aoe) ? b.bullet.spell.aoe : 0.5);
		}
				...
				if (Math.sqrt(X*X+Y*Y) <= rad) {
					...
					let newB = {delay: dd, born: born, time:b.bullet.spell.lifetime + LifetimeBonus, x:b.x+X, y:b.y+Y, vx:0, vy:0, xx:b.x+X, yy:b.y+Y, spriteID: KinkyDungeonGetEnemyID() + b.bullet.name+"Hit" + CommonTime(),
						bullet:{faction: b.bullet.faction, spell:b.bullet.spell, block: (b.bullet.blockhit ? b.bullet.blockhit : 0),
							...
							damage: {damage:b.bullet.spell.power, type:b.bullet.spell.damage, bind: b.bullet.spell.bind, time:b.bullet.spell.time}, lifetime: b.bullet.spell.lifetime + LifetimeBonus, name:b.bullet.name+"Hit", width:1, height:1}};
					KinkyDungeonBullets.push(newB);
...
function KinkyDungeonBulletsCheckCollision(bullet, AoE, force, d, inWarningOnly, delta) {
	...
	if (bullet.bullet.spell && bullet.bullet.spell.type == "dot") {
		if (bullet.bullet.spell.effectTileDoT) {
			KDCreateAoEEffectTiles(bullet.x, bullet.y, bullet.bullet.spell.effectTileDoT, bullet.bullet.spell.effectTileDurationModDoT, (bullet.bullet.spell.aoe) ? bullet.bullet.spell.aoe : 0.5);
		}
	}
	...
	if (bullet.bullet.damage && (bullet.time > 0 || force)) {
		if (bullet.bullet.aoe) {
			...
			for (let enemy of KinkyDungeonEntities) {
				let overrideCollide = (bullet.bullet.spell && bullet.bullet.alwaysCollideTags && bullet.bullet.alwaysCollideTags.some((tag) => {return enemy.Enemy.tags.has(tag);}));
				if ((enemy.x == bullet.x && enemy.y == bullet.y) && (bullet.reflected || overrideCollide
					|| (!bullet.bullet.spell || !bullet.bullet.faction
						|| (!KDFactionFavorable(bullet.bullet.faction, enemy) && (!bullet.bullet.damage || bullet.bullet.damage.type != "heal"))
						|| (!KDFactionHostile(bullet.bullet.faction, enemy) && (bullet.bullet.damage && bullet.bullet.damage.type == "heal"))
					))
